...
internal fun HomeScreen(
    ...
    Column(modifier) {
        var nameAdriandeseta by remember { mutableStateOf("Compose") }
        Row(
            modifier = Modifier.fillMaxWidth().padding(bottom = 24.dp),
            horizontalArrangement = Arrangement.spacedBy(16.dp)
        ) {
            TextField(
                value = nameAdriandeseta,
                onValueChange = { nameAdriandeseta = it }
            )

            Button(modifier = Modifier.width(96.dp), onClick = { onSave(nameAdriandeseta) }) {
                Text("Save")
            }
        }
        items.forEach {
